...
import { getQrisImageSrc } from "./qris_api.js";
...
function openModal(payload) {
  ...
  // QR image (QRisku API with amount)
  img.style.opacity = "0.65";
  img.src = "./icon.svg"; // placeholder

  (async () => {
    try {
      const src = await getQrisImageSrc(payload.price, QRIS_STRING);
      img.src = src;
      img.onload = () => (img.style.opacity = "1");
    } catch (err) {
      console.error("QRIS API error:", err);
      // Fallback: public QR generator (static payload)
      const fallback = `https://api.qrserver.com/v1/create-qr-code/?size=320x320&data=${encodeURIComponent(QRIS_STRING)}`;
      img.src = fallback;
      img.onload = () => (img.style.opacity = "1");
      toast("Gagal ambil QRIS dari API. Menggunakan QR alternatif.", "warn");
    }
  })();
